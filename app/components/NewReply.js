...
class NewReply extends Component {
  ...
  handleOnClick = e => {
    const { codeblock, codeType, repo, body } = this.state;
    e.preventDefault();
    this.props.newReply();
  };
    ...
    return (
      <MainContainer>
        <Form>
          ...
          <Button
            disabled={
              !body || Object.values(errors).some(val => !!val) ? true : false
            }
...
const mapState = ({ authentication, user, reposetories }) => ({
  authentication,
  user,
  reposetories,
});
...
export default connect(mapState, mapDispatch)(NewReply);
